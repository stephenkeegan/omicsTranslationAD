...
exercises: 10
questions:
- "How well do transcriptomic changes we observe in mouse models carrying AD-related mutations align with human AD data?"
...
Alzheimer's Disease (AD) is complex disease, we do not expect these mouse models to have complete late onset AD (LOAD) pathology. To study AD, we utilize MRI and PET imaging to match  back to the human imaging study [ADNI](https://adni.loni.usc.edu/), neuropathology information, and lots of genomics, proteomics, and metabolomics. These omics studies allow us to do real direct homology comparison between human and mouse since a significant portion of genes are  shared between these two species.
...
There are three independent human brain transcriptome studies. They are the Religious Orders Study and the Memory and Aging Project [(ROSMAP)](https://adknowledgeportal.synapse.org/Explore/Studies/DetailsPage/StudyDetails?Study=syn3219045), Mount Sinai Brain Bank [(MSBB)](https://adknowledgeportal.synapse.org/Explore/Studies/DetailsPage/StudyDetails?Study=syn3159438), and [Mayo RNAseq Study](https://adknowledgeportal.synapse.org/Explore/Studies/DetailsPage/StudyDetails?Study=syn5550404). Collected human postmortem brain RNA-seq data from seven distinct regions: dorsolateral prefrontal cortex (DLPFC), temporal cortex (TCX), inferior frontal gyrus (IFG), superior temporal gyrus (STG), frontal pole (FP), parahippocampal gyrus (PHG), and cerebellum (CBE), 

These postmortem samples are generally balanced for AD, MCI, and non-effected controls. This  provides a broad assessment how AD has affected multiple brain regions in 3 different populations around the US.
...
The Accelerating Medicines Partnership-Alzheimer’s Disease [(AMP-AD Consortium)](https://adknowledgeportal.synapse.org/Explore/Programs/DetailsPage?Program=AMP-AD)  has generated RNA-seq profiles from more than 1,200 human brains and is applying systems biology approaches toward the goal of elucidating AD mechanisms and potential therapeutic targets.

[Wan, et al.](https://doi.org/10.1016/j.celrep.2020.107908) performed meta analysis including all available AMP-AD RNA-seq datasets and systematically defined correspondences between gene expression changes associated with AD in human brains. Briefly, [Wan, et al.](https://doi.org/10.1016/j.celrep.2020.107908) performed library normalization and covariate adjustments for each human study separately using fixed/mixed effects modeling to account for batch effects. Among the 2978 AMP-AD modules identified across all tissues, 660 modules were selected which showed an enrichment for at least one AD-specific differential expressed gene set from the meta-analysis in cases compared to controls. 

Next, they performed multi method co-expression network analysis followed by differential analysis and found 30 co-expression modules related to LOAD pathology from a human cohort study. Among the 30 aggregate co-expression modules, five consensus clusters have been described by [Wan, et al.](https://doi.org/10.1016/j.celrep.2020.107908) These consensus clusters consist of a subset of modules which are associated with similar AD related changes across the multiple studies and brain regions. Further, they looked for enrichment of cell type signatures in these modules using expression-weighted cell type enrichment analysis [Skene and Grant, 2016](https://doi.org/10.3389/fnins.2016.00016) as well as  applied functional annotations to these modules.

The first module block is enriched in astrocytes, the next block is enriched in endothelial and microglial genes suggesting strong inflammation component, the third block in strongly enriched in neuron suggesting neurodegeneration, fourth is enriched in oligodendrocytes and glial genes suggesting myelination and finally mixed modules that have things to do like stress response and response to unfolded proteins. Stress response is not cell specific, so they may be throughout many cells in brain.


![AMP-AD GENE Modules](../fig/Picture1.png)

Here we are showing a matrix view of gene content overlap between these  modules, and you can see a few strongly overlapping group of modules, implicating similar pathology in different studies in different brain regions.

## Reading AMP-AD modules data

You can download data on the 30 human AMP-AD co-expression modules was obtained from the [Synapse data repository](https://www.synapse.org/#!Synapse:syn11932957/tables/). 
...
*column 2: human ensembl gene ID, 
...
We will only keep the columns of our interest and non-empty entries:
```{r}
...
Differential expression meta-analysis of reprocessed RNASeq data from AMP-AD (all 7 brain regions). LogFC [values](https://www.synapse.org/#!Synapse:syn11180450) for human transcripts were obtained via the AMP-AD knowledge portal.  
...
For our analysis, we need data for the "Diagnosis" model and comparisons between case vs controls. We subset the logFC data for these conditions. We need only three columns: "Tissue","Gene" and "logFC".  So, we will filter and subset the data.
...
Next, we will combine the fold change table `ampad_fc` and module table `ampad_modules`. First, look at both tables to check how can we merge them together?
...
**Reminder**: Every gene can be present in multiple brain regions but only one module from per brain region. Let's check that:
...
These appraoches allow us to assess directional coherence between AMP-AD modules and the effects of genetic perturbations in mice. In this lesson, we are going to use the first approach. 
Let's start!


#### Step 0: Reading Gene Expression Count matrix from Previous Lesson

We first read the result table saved after differential analysis in last [lesson](https://asliuyar.github.io/omicsTranslationAD/03-DESeq/index.html). We start with 5XFAD mouse model to understand all required steps to perform the correlation with human AD modules.
```{r FAD}
...
We compute a pearson correlation between changes in expression for each gene in a given module (log fold change for cases minus controls) with each mouse model (log fold change of the 5XFAD mice minus sex and age-matched B6 mice). 
...
Next, we compute correlation coefficients using the cor.test function built in R as following:
```{r}
cor.df <- df  %>%
          mutate(
...
#### Step 2: add signifcant correlations and join module cluster information to correlation table
...
Now, we will use the above matrix and visualize the correlation results using ggplot2 package.
```{r AMPAD1, message=FALSE, warning=FALSE, fig.width = 18, fig.height = 7, fig.show='hide'}
...
png(file="../results/AMPAD1.png", width=15, height=6, units="in", res = 1200)
ggplot2::ggplot() +
    ggplot2::geom_tile(data = data, ggplot2::aes(x = .data$module, y = .data$model_sex), colour = "black", fill = "white") +
    ggplot2::geom_point(data = dplyr::filter(data), ggplot2::aes(x = .data$module, y = .data$model_sex, colour = .data$correlation, size = abs(.data$correlation))) +
    ggplot2::geom_point(data = dplyr::filter(data, .data$significant), ggplot2::aes(x = .data$module, y = .data$model_sex, colour = .data$correlation),color="black",shape=0,size=9) +
...
In the above plot, the top row represents 30 AMP-AD modules grouped into 5 consensus clusters describing the major functional groups of AD-related alterations and the left column represents mouse models. Positive correlations are shown in blue and negative correlations in red. Color intensity and size of the circles are proportional to the correlation coefficient.  Black squares around dots represent significant correlation at p-value=0.05 and non-significant correlations are left blank. 
...
We also saw in the Volcano plot that immune-related genes were significantly overexpressed in the 5XFAD model. This significant positive correlation suggests that 5XFAD model captures inflammatory changes observed in human AD patients.

### Combing all steps into a function to perform correlation analysis

We can combine all the above steps performing correlation analyses into one function to use for other data by just inputting the log fold change matrix obtained from differential analysis.
...
We can use these functions directly into our other dataset from the LOAD1 cohort. 
...
We directly input the logFC table `DE_LOAD1.df` to `corr_function`:
```{r AMPAD2, message=FALSE, warning=FALSE}
...
range(correlation_for_plot$correlation)
```
```{r AMPAD_module_corPlot, fig.asp = 0.5, fig.width=16, fig.show='hide'}
magora_corrplot(correlation_for_plot,0.25)
```

```{r, echo=FALSE, warning=FALSE, include=FALSE}
png(file="../results/AMPAD_LOAD1.png", width=15, height=8, units="in", res = 1200)
magora_corrplot(correlation_for_plot,0.25)
dev.off()
```
